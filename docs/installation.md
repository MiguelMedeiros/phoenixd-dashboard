...
---
## Phoenixd API Authentication (Backend → Phoenixd)

The backend communicates with **phoenixd** using **HTTP Basic Authentication**.

- The API password is **automatically generated by phoenixd** on first startup
- The password is stored in the following file:
...
This password must be provided to the backend via the environment variable on .env:
```text
